...
import math, pygame, sys, os, copy, time, random, periodictable
...
BGCOLOR    = COMBLUE
COLORLIST = [RED, GREEN, YELLOW, ORANGE, BLUE, PURPLE, CYAN]

class Bubble(pygame.sprite.Sprite):
    def __init__(self, color, color_number, row=0, column=0):
        ...
        self.speed = 10
        self.color = color
        self.color_number = color_number #random.choice(list(periodictable.elements[random.randint(0, 18)])).symbol  
    ...
    def draw(self):
        ...
        text_surface = font.render(str(self.color_number), True, BLACK)
...
def main():
    ...
    pygame.display.set_caption('The Rebellion')
    MAINFONT = pygame.font.SysFont('Comic Sans', TEXTHEIGHT)
...
def stopBubble(bubbleArray, newBubble, launchBubble, score):
    deleteList = []
    popSound = pygame.mixer.Sound('POP.ogg')
    if newBubble.color_number == '-':
        for row in range(len(bubbleArray)):
            for column in range(len(bubbleArray[row])):
                if bubbleArray[row][column] != BLANK and bubbleArray[row][column].color == COLORLIST[0]:
    ...
    for row in range(len(bubbleArray)):
        for column in range(len(bubbleArray[row])):
            if bubbleArray[row][column] != BLANK and newBubble is not None:
                if (pygame.sprite.collide_rect(newBubble, bubbleArray[row][column])) or newBubble.rect.top < 0:
